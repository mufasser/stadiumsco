 ...
 * Slug: vip-football-theme/tournament-event-list
 * Categories: query
...
$tournamentID = getCurrentPostId();
$tournament_id = getTournamentApiId($tournamentID);
$tournamentBackgroundImage = get_field('background_image',$tournamentID);
$tournamentLogo = getThumbnailByPostId($tournamentID);

?>
<section class="w-screen h-42 bg-[url('<?php echo $tournamentBackgroundImage; ?>')]">  
    <section class="align-middle p-8 mb-10 w-full has-global-padding is-layout-constrained wp-block-group-is-layout-constrained">
        <a href="#" class="flex shadow-xl min-w-0 w-26 md:w-30 sm:w-32 tournament-logo align-middle text-center justify-center rounded-lg bg-white/[0.2] border-1 border-gray-200/50 w-32 mx-auto">
            <img src="<?php echo $tournamentLogo[0];?>" alt="">
        </a>
        <div>
            <h1 class="text-center text-3xl font-bold text-white"><?php echo get_the_title($tournamentID) ?></h1>
        </div>
    </section>

</section>
<section class="w-full has-global-padding is-layout-constrained wp-block-group-is-layout-constrained">

<div id="eventScrollContainer" class="flex flex-wrap items-start pb-8">
    ...
    <?php 
        ...
        if ( $the_query->have_posts() ) {
            ...
            <?php
                ...
                <div class="flex-none event-wrapper mb-8 w-96 md:w-96 sm:w-32 mr-8 md:pb-4 border:1px|solid|gray-300 bg-white shadow-lg transition duration-300 ease-in-out hover:shadow-xl relative rounded-lg">
